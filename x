...
Mouse.KeyDown:Connect(function(Key)
    -- Handle targeting when Type = "keybind"
    if getgenv().CamLock.Type == "keybind" then
...
RunService.RenderStepped:Connect(function()
    -- If Type = "auto", target automatically based on radius
    if getgenv().CamLock.Type == "auto" then
        if CamLock.Normal.Enabled then
            local closestTarget = ClosestPlrFromMouse()  -- Get the nearest player in the radius
            if closestTarget and closestTarget.Character then
                -- If the closest player meets the health criteria, lock onto them
                local humanoid = closestTarget.Character:FindFirstChildOfClass("Humanoid")
                if humanoid and humanoid.Health >= 7 then
                    TargetPlayer = closestTarget
                    IsTargeting = true
                end
            end
        end
    elseif getgenv().CamLock.Type == "keybind" then
        -- Existing targeting logic for keybind mode
...
RunService.RenderStepped:Connect(function()
    ...
    -- Handle Silent startup and targeting based on the CamLock type
    if getgenv().Silent.Startup.Enabled then
        if getgenv().CamLock.Type == "auto" then
            -- Auto target the closest player within radius
            if CamLock.Normal.Enabled then
                local closestTarget = ClosestPlrFromMouse()  -- Get the nearest player
                if closestTarget and closestTarget.Character then
                    local humanoid = closestTarget.Character:FindFirstChildOfClass("Humanoid")
                    if humanoid and humanoid.Health >= 7 then
                        TargetPlayer = closestTarget
        ...
        elseif getgenv().CamLock.Type == "keybind" and IsTargeting then
            -- Handle keybind mode (targeting already selected target)
            if TargetPlayer and TargetPlayer.Character then
    ...
    -- If the target was lost (for either auto or keybind mode) and goes behind a wall, attempt to retarget
...
-- Delayed loop for FOV visibility update
task.spawn(function()
